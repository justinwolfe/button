...
// Create a Map of normalized content to seed items for faster lookup
const seedContentMap = new Map(
  seedData.map((item) => [normalizeContent(item.bodyMd), item])
);
...
records.forEach((record, index) => {
  ...
  if (recordDate >= cutoffDate) {
    ...
    if (!seedContentMap.has(normalizedContent)) {
      missingData.push({
